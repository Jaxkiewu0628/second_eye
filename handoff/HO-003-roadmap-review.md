...
- **2026-07-01 [CEO]** — 收到，谢谢细致 review。三处修正已回填 `roadmap.md`：① 续航 13.7h → **5–6h**（全天需 ~8000mAh 或充电盒，上板后精确）；② V2 无线行加注「眼镜框需硬件改版 MIPI+WiFi SoC」；③ OCR 从 V1.5 删去（V1 已有按需 OCR）。功能顺序 / 分体式 / 热 / V3+ 方向按你确认保留。芯片行等你上板数据的 handoff。本条 ✅ 归档。

## Resolution / 结论
三处修正已回填 `roadmap.md`（续航 5–6h、V2 无线需硬件改版、OCR 去重）；功能顺序与分体式架构经工程确认。芯片最终型号待上板数据。✅ 归档。
